...
if [[ ! $AUR_HELPER == none ]]; then
  ...
  sed -n '/'$INSTALL_TYPE'/q;p' ~/PintArch/pkg-files/aur-pkgs.txt | while read line
  do
...
if [[ $DESKTOP_ENV == "DWM" ]]; then
  ...
  -------------------------------------------------------------------------
  "
  sudo pacman --noconfirm --needed -S xorg xorg-xinit xorg-xclipboard pipewire pipewire-pulse pipewire-alsa alsa-utils pulsemixer pamixer pcmanfm gvfs
  ...
  sudo bash -c 'echo export QT_QPA_PLATFORMTHEME=\"qt6ct\" >> /etc/environment'
...
if [[ $INSTALL_TYPE == "TOTAL" ]]; then
  ...
  if [[ $DESKTOP_ENV == "DWM" ]]; then
    echo Virt MAnager - GUI para QEMU/KVM
    # install visual enviroment for QEMU/KVM
